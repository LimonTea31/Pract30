...
public class MainActivity4 extends AppCompatActivity {
    ...
    public void onClickStart(View v) {
        ...
        //Intent intent2 = new Intent(this, MyService4.class);

        pi = createPendingResult(TASK1_CODE, intent, 0);
        intent = new Intent(getApplicationContext(), MyService4.class).putExtra(PARAM_TIME, 7)
                .putExtra(PARAM_PINTENT, pi);
        startService(intent);

        pi = createPendingResult(TASK1_CODE, intent, 0);
        intent = new Intent(getApplicationContext(), MyService4.class).putExtra(PARAM_TIME, 4)
                .putExtra(PARAM_PINTENT, pi);
        startService(intent);

        pi = createPendingResult(TASK1_CODE, intent, 0);
        intent = new Intent(getApplicationContext(), MyService4.class).putExtra(PARAM_TIME, 6)
                .putExtra(PARAM_PINTENT, pi);
        startService(intent);
    }
